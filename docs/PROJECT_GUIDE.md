...
### Aug 2025
- **v0.8.0 - LLM-Enhanced Regime-Adaptive Strategy**: Revolutionary upgrade implementing LLM-enhanced trading with QwQ-32B integration. Added RegimeAdaptiveStrategy combining AI regime detection with rule-based fallbacks, dynamically switching between momentum (E3) and contrarian (FundingFade) approaches. Enhanced AI prompts for regime classification (bull_trend, bear_trend, high_vol, crash, chop) and position sizing recommendations. Implemented dynamic leverage system (2x-3x) based on market regime with proper risk management integration. Added 0.1% per-side fee modeling to backtests for realistic performance assessment. Created LLM-aware optimization configuration with AI confidence weighting and early stopping rules. Fixed leverage cap calculation in risk manager and integrated regime-aware position sizing.
- **v0.8.1 - Enhanced Risk Management, Leverage Optimization & Modular Architecture**: Major upgrade implementing real Drift equity integration, ATR-based exit management, leverage optimization, and complete modular architecture refactoring. Bot now uses live account monitoring ($83.58 USDC trading collateral), progressive profit taking (1R/2R), time-based exits, and comprehensive PnL logging. Position sizing increased to $22 (0.26x leverage) with enhanced risk management. Added complete LoRA training infrastructure and modernized development workflow with TypeScript path mapping, ESLint + Prettier integration, organized npm scripts, and quality assurance tooling. Implemented full modular architecture with core types system, strategy framework, risk management, data layer abstraction, AI provider system, and centralized configuration management. All exits use reduce-only orders for safety.
