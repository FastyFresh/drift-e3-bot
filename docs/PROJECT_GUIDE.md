...
### Aug 2025
- **v0.8.0 - Enhanced Risk Management, Leverage Optimization & Modern Development**: Major upgrade implementing real Drift equity integration, ATR-based exit management, leverage optimization, and modern development tooling. Bot now uses live account monitoring ($83.58 USDC trading collateral), progressive profit taking (1R/2R), time-based exits, and comprehensive PnL logging. Position sizing increased to $22 (0.26x leverage) with enhanced risk management. Added complete LoRA training infrastructure and modernized development workflow with TypeScript path mapping, ESLint + Prettier integration, organized npm scripts, and quality assurance tooling. All exits use reduce-only orders for safety.
