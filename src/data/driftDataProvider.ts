...
export class DriftDataProvider {
  ...
  private async downloadIfNotCached(market: string, type: string, date: string): Promise<string> {
    const url = `${this.baseUrl}/market/${market}/${type}Records/${date.slice(0, 4)}/${date}`;
    const cacheFile = path.join(this.cacheDir, `${market}_${type}_${date}.json`);
  ...
  private aggregateTradesToCandles(trades: any[], resolutionSec: number): Candle[] {
    const candles: Candle[] = [];
    const grouped: Record<string, any[]> = {};

    for (const trade of trades) {
      const ts = (trade.blockTimestamp ?? trade.ts ?? trade.slot * 400) * 1000; // fallback approx
  ...
  async load(
    ...
    for (const date of dateRange) {
      try {
        const tradeFile = await this.downloadIfNotCached(market, 'trade', date);
        const tradeData = this.safeParseFile(tradeFile);
        console.log(`[DriftDataProvider] ${market} ${date} trades=${tradeData.length}`);
        const candles = this.aggregateTradesToCandles(tradeData, resolutionSec);
        console.log(`[DriftDataProvider] ${market} ${date} candles=${candles.length}`);

        // attempt funding
        let fundingRate;
        try {
          const fundingFile = await this.downloadIfNotCached(market, 'fundingRate', date);
          const fundingData = this.safeParseFile(fundingFile);
          fundingRate =
            Array.isArray(fundingData) && fundingData.length
  ...
  private safeParseFile(filePath: string): any[] {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        console.log(
          `[safeParseFile] ${path.basename(filePath)} parsed=${linesTotal} accepted=${tradesAccepted} skipped=${linesSkipped}`
        );
        return filtered;
      }
      if (accept(parsed)) {
  ...
  private mapTrade(obj: any): any | null {
    ...
    if (typeof trade === 'object' && 'ts' in trade && 'price' in trade === false) {
      ...
      const mapped: any = {
        price: Number(
          trade.markPriceTwap || trade.oraclePriceTwap || trade.oraclePrice || trade.price
        ),
        ts: Number(trade.ts),
    ...
    return {
      ...
      ts,
      baseAmount:
        typeof trade.baseAmount === 'number' && isFinite(trade.baseAmount) ? trade.baseAmount : 0,
